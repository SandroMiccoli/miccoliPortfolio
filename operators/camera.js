(function (root) {
	...
	root.SynthRegistry.register({
		...
		color: cat.color,
		help: 'Live camera texture. Display uses the USB webcam (C270) on the renderer, or RealSense IR (GREY via the Pi server). Chromium cannot open the IR node directly. Depth is hidden. Phone uses this control phone and sends frames over the network. Device, Mirror, and Fit appear after that source is live. Reconnect stops and reopens the camera.',
		implemented: true,
		defaults: {
			source: 'display',
			deviceId: '',
